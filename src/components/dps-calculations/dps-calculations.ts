...
@Component({
  selector: 'dps-calculations',
  templateUrl: 'dps-calculations.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class DPSCalculationsComponent implements OnInit, OnDestroy {
  ...
  @Input('DPSGridInput') DPSGridInput                : Array<Array<any>> = []                ;
  @Output('DPSCalculationsGrid') DPSCalculationsGrid = new EventEmitter<Array<Array<any>>>() ;
  ...
  ngOnInit() {
    Log.l('DPSCalculationsComponent: ngOnInit() fired');
...
    // unsubscribe to ensure no memory leaks
    Log.l('DPSCalculationsComponent: ngOnDestroy() fired');
  ...
  public runWhenReady() {
    this.dataGrid = this.DPSGridInput || [];
  ...
  public installSubscribers() {
    this.subscription = this.dispatch.periodUpdated().subscribe((periodObject: any) => {
      Log.l("DPSCalculationsComponent: received Observable update for PayrollPeriod!\n", periodObject.period);
      let period:PayrollPeriod = periodObject.period;
      this.updateView('update');
      // this.updatePeriod(period);
    });
    this.gridSubscription = this.dispatch.dpsCalculationsGridUpdated().subscribe((gridObject: any) => {
      Log.l("DPSCalculationsComponent: received Observable update for dataGrid!\n", gridObject.grid);
      let grid:Array<Array<any>> = gridObject.grid;
      this.dataGrid = grid;
      this.updateView('update');
    });
  }
  ...
  public generateDPSCalculations(period:PayrollPeriod) {
    ...
    this.DPSCalculationsGrid.emit(datagrid);
  }
  ...
  public generateGridData() {
    ...
    for(let entry of e) {
      ...
      for(let shift of shifts) {
        ...
        let profitPercent:Decimal = profit.div(revenue);
        let row:Array<any> = [
          /* 0  */ i,
