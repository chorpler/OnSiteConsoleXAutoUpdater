...
export class NumberService {
  ...
  public toCurrency(value:any, decimalPlaces?:number, currencyName?:string, currencyFractionName?:string) {
    ...
    let decimal = new BigNumber(bn.minus(integer).absoluteValue().toFixed(precision));

    let denominator = new BigNumber("1");
    // let decimalValue = integer.minus(decimal);
    let decimalValue = new BigNumber(decimal);
    while (!decimalValue.equals(decimalValue.trunc())) {
      denominator = denominator.times(10);
      decimalValue = decimal.times(denominator);
    }
    let bigDenominator = new BigNumber(10).toPower(precision);
    if(!denominator.equals(bigDenominator)) {
    ...
    if (!decimalValue.equals(0) || precision) {
  ...
  public generateWords(num:any, wordArray?:any) {
    ...
    let remainder = new BigNumber(0), word = "", words = wordArray ? wordArray : [];
