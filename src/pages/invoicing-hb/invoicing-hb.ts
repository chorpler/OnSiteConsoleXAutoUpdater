...
import { InvoiceHBComponent                                                              } from 'components/invoice-hb/invoice-hb' ;
...
@IonicPage({name: "Invoicing HB"})
@Component({
  selector: 'page-invoicing-hb',
  templateUrl: 'invoicing-hb.html',
})
export class InvoicingHBPage implements OnInit,OnDestroy {
  @ViewChild('invoiceHB') invoiceHB:InvoiceHBComponent;
  ...
  constructor(
    ...
    window["onsiteinvoicinghb"] = this;
  }

  ngOnInit() {
    Log.l('InvoicingHBPage: ngOnInit() fired');
  ...
  ngOnDestroy() {
    Log.l("InvoicingHBPage: ngOnDestroy() fired");
  ...
  public runWhenReady() {
    this.keySubscription = this.keyService.commands.subscribe((command:Command) => {
      switch(command.name) {
        // case "InvoicingHBPage.showOptions"      : this.showOptions(); break;
      }
    });
