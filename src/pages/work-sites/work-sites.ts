...
import { Street, Address, Jobsite                                    } from 'domain/domain-classes'      ;
...
export class WorkSitesPage implements OnInit,OnDestroy {
  ...
  public get prefs()      : any { return WorkSitesPage.PREFS; }               ;
  ...
  public ionViewDidLoad() {
    Log.l('ionViewDidLoad WorkSitesPage');
  }

  public ionViewWillEnter() {
    Log.l("ionViewWillEnter WorkSitesPage");
  ...
  public runWhenReady() {
    ...
    this.updateJobsiteList().then((res) => {
      ...
      Log.l("WorkSites: ionViewWillEnter() threw error while trying to update job site list!");
      Log.e(err);
    });
  }
    ...
    return new Promise((resolve,reject) => {
      ...
      if(this.jobsites && this.jobsites.length) {
        ...
        this.server.getJobsites().then((docs: Array<any>) => {
          ...
          this.jobsites = jobsites.sort((a,b) => {
            return a.sort_number < b.sort_number ? -1 : a.sort_number > b.sort_number ? 1 : 0;
          });
          resolve(this.jobsites);
        }).catch((err) => {
