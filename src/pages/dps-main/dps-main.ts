import { DPSReportComponent, DPSCalculationsComponent,                        } from 'components/component-classes'         ;
import { DPSAncillaryCalculationsComponent, DPSDailySummaryComponent          } from 'components/component-classes'         ;
...
import { Log, Moment, moment, oo, dec, Decimal, _matchCLL, _matchSite         } from 'config/config.functions'              ;
import { Jobsite, Employee, Report, ReportOther, PayrollPeriod, Shift         } from 'domain/domain-classes'                ;
import { Schedule, Invoice, DPS                                               } from 'domain/domain-classes'                ;
import { OSData                                                               } from 'providers/data-service'               ;
import { PDFService                                                           } from 'providers/pdf-service'                ;
import { OptionsComponent                                                     } from 'components/options/options'           ;
import { MenuItem, SelectItem,                                                } from 'primeng/primeng'                      ;
import { DataTable, OverlayPanel, Spinner                                     } from 'primeng/primeng'                      ;
...
export class DPSMainPage implements OnInit,OnDestroy {
  ...
  public settings:any = {
    get dps():DPS { return OSData.dps; },
    get days_in_month():number { return OSData.dps.days_in_month; },
    set days_in_month(value:number) { OSData.dps.days_in_month = value; },
    get working_techs():number { return OSData.dps.working_techs; },
    set working_techs(value:number) { OSData.dps.working_techs = value; },
    get multiplier():number { return OSData.dps.multiplier.toNumber(); },
    set multiplier(value:number) { let val = value || 0; OSData.dps.multiplier = new dec(val) },
    get cost_modifier():number { return OSData.dps.cost_modifier.toNumber(); },
    get internal_salaries():number { return OSData.dps.internal_salaries.toNumber(); },
    set internal_salaries(value:number) { OSData.dps.internal_salaries = new dec(value); },
    get monthly_fuel():number { return OSData.dps.fuel.toNumber(); },
    set monthly_fuel(value:number) { OSData.dps.fuel = new dec(value); },
    get monthly_transportation():number { return OSData.dps.transportation.toNumber(); },
    set monthly_transportation(value:number) { OSData.dps.transportation = new dec(value); },
    get insurance_travelers():number { return OSData.dps.travelers.toNumber(); },
    set insurance_travelers(value:number) { OSData.dps.travelers = new dec(value);},
    get insurance_imperial():number { return OSData.dps.imperial.toNumber(); },
    set insurance_imperial(value:number) { OSData.dps.imperial = new dec(value); },
    get insurance_tx_mutual():number { return OSData.dps.tx_mutual.toNumber(); },
    set insurance_tx_mutual(value:number) { OSData.dps.tx_mutual = new dec(value); },
    get insurance_blue_cross():number { return OSData.dps.blue_cross.toNumber(); },
    set insurance_blue_cross(value:number) { OSData.dps.blue_cross = new dec(value); },
    get insurance_property():number { return OSData.dps.property.toNumber(); },
    set insurance_property(value:number) { OSData.dps.property = new dec(value); },
  ...
  /* Getters and Setters */
  public get dps():DPS                       { return OSData.dps;                                         };
  public set dps(value:DPS)                  { OSData.dps = value;                                        };
  ...
  public initializeTabs() {
    this.tabs = [
      { item: 0, header: "DPS Report"                 , icon: "fa-table" , selected: true , content: "DPS Report"                 },
      { item: 1, header: "DPS Calculations"           , icon: "fa-table" , selected: false, content: "DPS Calculations"           },
      { item: 2, header: "DPS Ancillary Calculations" , icon: "fa-table" , selected: false, content: "DPS Ancillary Calculations" },
      { item: 3, header: "DPS Daily Summary"          , icon: "fa-table" , selected: false, content: "DPS Daily Summary"          },
  ...
  public generateAncillaryData() {
    ...
    for(let ins in insurances) {
      ...
      mInsurance = mInsurance.add(value);
    }
    ...
    for(let row of calcGrid) {
      expenses          = expenses.add(row[calcRows.expenses])                   ;
      vacation          = vacation.add(row[calcRows.vacation])                   ;
      training          = training.add(row[calcRows.training])                   ;
      travel            = travel.add(row[calcRows.travel])                       ;
      standby           = standby.add(row[calcRows.standby])                     ;
      transportation    = transportation.add(row[calcRows.transportation])       ;
      fuel              = fuel.add(row[calcRows.fuel])                           ;
      insurance         = insurance.add(row[calcRows.insurance])                 ;
      perdiem           = perdiem.add(row[calcRows.perdiem])                     ;
      internal_salaries = internal_salaries.add(row[calcRows.internal_salaries]) ;
      lodging           = lodging.add(row[calcRows.lodging])                     ;
      payroll           = payroll.add(row[calcRows.payroll])                     ;
    }
    ...
    for(let row of grid) {
      totalTechCosts = totalTechCosts.add(row[1]);
    }
  ...
  public generateDPSCalculationsGridData() {
    ...
    let grid = [];
    let i = 0;
    let working_techs = this.settings.working_techs;
  ...
  public createDPSDailySummaryDataGrid() {
    ...
    for(let site of sites) {
      ...
      let siteTotals1 = {
        ...
        get percent() { return this.billed.equals(0) ? this.zero : this.profit.div(this.billed).times(100); },
      };
      let siteTotals2 = {
        name: site.schedule_name,
        number: site.site_number,
      ...
      for(let tech of techs) {
        ...
        let percent = billing.equals(0) ? new dec(0) : new dec(profit1).div(billing).times(100);
        let percent2 = billing.equals(0) ? new dec(0) : new dec(profit2).div(billing).times(100);
      ...
      // siteTotals.percent = siteTotals.billed.equals(0) ? new dec(0) : new dec(siteTotals.profit).div(siteTotals.billed).times(100);
    ...
    // siteTotals.percent = siteTotals.billed.equals(0) ? new dec(0) : siteTotals.profit.div(siteTotals.billed).times(100);
    this.grid1 = grid1;
    this.grid2 = grid2;
    this.DSGrid = grid1;
  ...
  public updateWorkingTechCount() {
    ...
    if(mode === 'reports') {
      for(let row of grid) {
        if(row.length > 2) {
          if(!row[rowval.billing].equals(0)) {
  ...
  public moneyAdd(event:any, key:string) {
    ...
    if(key === 'days_in_month' || key === 'working_techs') {
      let newValue:Decimal = value.add(amount);
