...
export class SchedulePrintBetaPage {
  ...
  ionViewDidLoad() {
    Log.l('ionViewDidLoad SchedulePrintBetaPage');
    this.data.appReady().then(res => {
      ...
      if(this.schedule) {
        ...
        Log.l("SchedulePrintBetaPage: not ready, should go to home page.");
  ...
  public generateGridData() {
    ...
    let techs = this.techs.slice(0).filter(a => a.locID.toUpperCase() !== 'OFFICE');
  ...
  public generateHeaderData() {
    ...
    for(let client of clients) {
      if(client.name === 'XX' || client.name === 'AA') {
        continue;
      }
