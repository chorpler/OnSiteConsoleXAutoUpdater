...
import { Log, Moment, moment, oo, _matchCLL, _matchReportSite                            } from 'config/config.functions'            ;
...
export const MAX_LINES:number = 25;
export const SUMMARY_LINES:number = 10;

@IonicPage({name: "Invoicing Keane"})
@Component({
  selector: 'page-invoicing-kn',
  templateUrl: 'invoicing-kn.html',
})
export class InvoicingKNPage implements OnInit,OnDestroy {
  ...
  public title             : string               = "Invoicing Keane Group"         ;
  ...
  public getCrewList(rpts?:Report[]) {
    ...
    let regex1 = /^crew/i;
    let regex2 = /[1-9]?\d/;
    let reports = reportList.filter((a:Report) => {
      // return a.work_order_number.indexOf("CREW") > -1;
      let out = a.work_order_number.match(regex1);
      return out ? true : false;
    });
    let crewObject:any = {};
    for(let report of reports) {
      let crewField = report.work_order_number;
      let allDigits = crewField.match(regex2);
      if(allDigits) {
        for(let digits of allDigits) {
          let num = Number(digits);
          // let key = sprintf("CREW %02d", num);
          crewObject[num] = 1;
        }
      }
    }
    let crewList:any[] = Object.keys(crewObject);
    crewList = crewList.map(a => Number(a));
    crewList = crewList.sort((a,b) => a > b ? 1 : a < b ? -1 : 0);
