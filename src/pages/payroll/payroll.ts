...
export type PPS = {site:Jobsite, period:PayrollPeriod};
...
export class PayrollPage implements OnInit,OnDestroy {
  ...
  public get prefs():any { return PayrollPage.PREFS;} ;
  ...
  ngOnInit() {
    ...
    //   Log.l("Scheduling: got hotkey:\n", event, combo);
    //   // this.showOptions();
    //   this.showOptions(this.content);
  ...
  public generatePayrollData() {
    return new Promise((resolve,reject) => {
      ...
      let _sortReports = (a:Report|ReportOther, b:Report|ReportOther) => {
      ...
      this.employees = this.allData.employees.filter((a:Employee) => {
        // return obj.active && obj['client'] && obj['location'] && obj['locID'] && obj['roles'][0].toLowerCase() !== 'manager' && obj['userClass'][0].toLowerCase() !== 'manager';
  ...
  public updatePeriod(period:PayrollPeriod) {
    ...
    let spinnerID = this.alert.showSpinner(text);
    this.loading = this.alert.getSpinner(spinnerID);
    this.changePeriod(period).then(res => {
      return this.alert.hideSpinnerPromise(spinnerID);
    }).then(res => {
+      Log.l("updatePeriod(): Successfully updated, now running change detection...");
  ...
  public periodChanged(period:PayrollPeriod) {
    ...
    this.others = this.allData.others.filter((a:ReportOther) => {
      let report_date = a.report_date.format("YYYY-MM-DD");
      return report_date >= start && report_date <= end;
    });
  ...
  public viewTechReports(period:PayrollPeriod, tech:Employee) {
    ...
    let modal = this.modalCtrl.create('Shift Reports', {tech:tech, period:period, mode:0}, {cssClass: 'shift-reports-modal'});
  ...
  public viewTechReportsForPeriod(period:PayrollPeriod, tech:Employee) {
    ...
    let modal = this.modalCtrl.create('Shift Reports', {tech:tech, period:period, mode:1}, {cssClass: 'shift-reports-modal'});
    modal.onDidDismiss(data => {
      Log.l("All Period Reports: dismissed.");
      if(data) {
        Log.l("data is:\n", data);
  ...
  public createExportData() {
    ...
    for(let tech of this.employees) {
      ...
      // let shiftLen = tech.shiftLength;
      // let shiftStart = tech.shiftStartTime;
      let rotation = this.data.getTechRotationForDate(tech, pDate);
      let rotSeq   = this.data.getRotationSequence(rotation);
      let techPeriod = this.ePeriod.get(tech);
      let shifts = techPeriod.getPayrollShifts();
  ...
  public async refreshData(event?:any) {
    // this.notify.addInfo("NOPE", "Yeah, right. Like there would be a refresh function so fast. Dream on, Mike!", 10000);
    try {
