...
function fixPathSlashes(p: string) {
    ...
    console.log(`fixPathSlashes(): returning '${replaced}'`);
    return replaced;
}
...
function injectScriptToWebView(target: Electron.WebviewTag, opts: InPageSearchOptions) {
    ...
    console.log("injectedScript(): file is: ", injected_script);
    ...
    //     // s.src = 'file:///C:/code/tron/OnSiteConsoleX/www/${injected_script}';
    //     s.src = 'file://search-window.js';
...
export class InPageSearch extends EventEmitter {
    ...
    closeSearchWindow() {
      console.log("closeSearchWindow() running");
        if (!this.opened) {
            log('Already closed');
            return;
        }
